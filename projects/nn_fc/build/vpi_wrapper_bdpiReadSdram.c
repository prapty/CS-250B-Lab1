 ...
 *     #include "vpi_wrapper_bdpiReadSdram.h"
 *     void (*vlog_startup_routines[])() = { bdpiReadSdram_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiReadSdram call=bdpiReadSdram_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiReadSdram vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int bdpiReadSdram(unsigned int );

/* VPI wrapper function */
PLI_INT32 bdpiReadSdram_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  unsigned int arg_1;
  ...
  vpi_result = bdpiReadSdram(arg_1);
...
/* sft: $imported_bdpiReadSdram vpiSysFuncSized 32 unsigned */

/* tab: $imported_bdpiReadSdram call=bdpiReadSdram_calltf size=32 acc=rw:%TASK */

PLI_INT32 bdpiReadSdram_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}

/* VPI wrapper registration function */
void bdpiReadSdram_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiReadSdram";
  tf_data.calltf = bdpiReadSdram_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiReadSdram_sizetf;
  tf_data.user_data = "$imported_bdpiReadSdram";
